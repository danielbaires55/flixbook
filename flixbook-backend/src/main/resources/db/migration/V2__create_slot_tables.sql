...
CREATE UNIQUE INDEX uq_slot_medico_start ON slot(medico_id, data_ora_inizio);
CREATE INDEX idx_slot_blocco ON slot(blocco_orario_id);
CREATE INDEX idx_slot_medico_date ON slot(medico_id, data_ora_inizio);

-- Appointment to slot (1-1, because prestazioni durano 30 min)
ALTER TABLE appuntamenti 
  ADD COLUMN slot_id BIGINT UNIQUE,
